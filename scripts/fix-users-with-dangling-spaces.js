...
getAllDanglingUsers()
    ...
    .then(({ autoFixableUsers, samePersonUsers, differentPersonUsers }) => {
        console.log(
            `total # of dangling users: ${autoFixableUsers.length + samePersonUsers.length + differentPersonUsers.length}`,
        );
...
function determineFixabilities(danglingUsers, differentPersonUsers = [], samePersonUsers = [], autoFixableUsers = []) {
    return new Promise((resolve, reject) => {
        ...
        determineUserFixability(danglingUser).then((fixability) => {
            if (fixability === 'auto') {
